...
表示幅に合わせて複数サイズを書き出す（`widths` と `sizes`）。固定の1枚だけを出すと、CMSに上がる大きな画像（スマホ撮影の4000px級）がそのまま配信され、表示に必要な量の10倍以上を読み込むことになる。

`sizes` に書く値は `src/styles/global.css` の実測から決める。コンテンツ幅は `--content-max-width: 800px` から左右のパディング `clamp(1.25rem, 5vw, 2rem)` を引いた最大736px。

| 場所 | 表示幅 | 指定 |
|---|---|---|
| ヒーロー画像 | コンテンツ幅いっぱい（最大736px） | `widths={[736, 1472]}` / `sizes="(min-width: 800px) 736px, 100vw"` |
| 一覧カード | 2カラム時390px（736px − gap 20px の半分 ＋ カードのpadding分32px）、767px以下の1カラム時はほぼ画面幅 | `widths={[400, 800, 1200]}` / `sizes="(min-width: 768px) 390px, 100vw"` |

`widths` は等倍・2倍（カードは3倍まで）の画面をまかなう値。元画像より大きい値を書いても引き伸ばされず、元画像の幅で打ち止めになる（1200px幅の画像に `1472` を指定すると `1200w` が出力される）。

`width` / `height` 属性には元画像の寸法がそのまま入る。縦横比は変わらないため、レイアウト移動の解消には影響しない。カードの枠は従来どおりCSSの `aspect-ratio` が確保する。
