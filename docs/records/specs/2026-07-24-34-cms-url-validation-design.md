...
1. `npm run build` が成功する（受け入れ条件）。
2. あわせて `npm run build && npm run check` を全パス（エラー・warning 0件）で確認する。このプロジェクトは npm（`package-lock.json`）を使い、`package.json` の scripts は `build` / `check`（`astro check`）/ `dev` / `preview` / `astro` のみ。テスト・lint・独立typecheckスクリプトは存在しないため、共通CLAUDE.mdの汎用検証コマンドではなくこの2つを品質ゲートとする。
