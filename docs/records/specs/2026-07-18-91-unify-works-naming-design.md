...
**全廃する語**: `制作物`、`プロジェクト`、`Projects`、`projects`（コード識別子含む）。
...
- `docs/coding-standards.md` の現構成説明を works 系に更新：ディレクトリツリーの `projects/`・`ProjectCard.astro`、命名表の `ProjectCard.astro`、型例 `ProjectEntry` / `PageType = ... | "projects"` / `getPublishedProjects` / `getCollection("projects")` 例をすべて works 系へ。
...
1. **残存ゼロの確認**: `grep -rniE "project|制作物|プロジェクト" src public docs/coding-standards.md` がヒット0件
